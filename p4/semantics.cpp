...
void Semantics::generate(NodeT* node)
{
	...
	if (node->label == "<loop>") {
		...
		carry_labels_num = current_labels_num++;
		//	printf("Here");
		if (operator_map[node->c2->tokens[0].value] == "modulusTk") {
	//		printf("Here");
			file << "MULT _T" << vars_num << endl;
	...
	if (node->label == "<RO>") {
		/* CHECK: < | < < | < > */
		cout << operator_map[node->tokens[0].value] << endl;
		if (operator_map[node->tokens[0].value] == "equalLessTk") {
			
			file << "BRNEG _L" << carry_labels_num << endl;
		}
		/* CHECK: > | > > */
		else if (operator_map[node->tokens[0].value] == "equalGreaterTk") {
			file << "BRPOS _L" << carry_labels_num << endl;
		}
		/* CHECK: == */
		...
		else if(operator_map[node->tokens[0].value] == "leftBracketTk") {
